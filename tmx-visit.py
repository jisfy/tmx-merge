...
class JumpCalculationData:
	...
	def get_max_jump_height_tiles(self, tilesize):
		...
		print "~~~~~~ MAX JUMP HEIGHT %s , %s , %s, %s" % (str(self.max_jump_height), str(max_jump_height_pixels), str(max_jump_height_tiles), str(tilesize))
...
def find_projection(grid, neighbor_position, max_jump_height, tilemap_size):
	projection = None
...
def find_right_projection(grid, position, max_jump_height, tilemap_size):
	right_neighbor_position = (position[0], position[1] + 1)
	return find_projection(grid, right_neighbor_position, max_jump_height, tilemap_size)
...
def find_jump_candidates(grid, source_position, tilemap, jump_calculation_data):
	...
	for horizontal_distance in range(1, jump_calculation_data.max_jump_horizontal_tiles):
		horizontal_nth_neighbor_col_index = source_position[1] + horizontal_distance		
		projection_found = find_right_projection(grid, (source_position[0], horizontal_nth_neighbor_col_index), jump_calculation_data.get_max_jump_height_tiles(tilemap.tile_size), tilemap.size)
		if (projection_found != None):
...
def add_projected_navpoints_new(tilemap, grid, first_navpoint_id, jump_calculation_data):
	...
	for col_index in range(tilemap.size[0]):
		...
		for row_index in range(tilemap.size[1]):
			...
			if (cell_is_platform) and (should_have_right_projection(grid, (row_index, col_index), tilemap.size)):	
				projection_found = find_jump_candidates(grid, (row_index, col_index), tilemap, jump_calculation_data)
				# print " should have right r:" + str(row_index) + ", c:" + str(col_index) + ",p:" + str(projection_found)				
...
def get_jump_and_falling_height_for_obstacle(obstacle_height, character_height, extra_jump_height_percent):
	...
	print "********* jump height " + str(jump_height_extra) 
...
def get_maximum_tiles_horizontal(max_jump_height, step_period, step_walk_speed, step_world_gravity, tilesize, pixels_per_meter):
    ...
    print "maximum_full_tiles %s, partial_tile_pixesls %s, total_tiles %s" % (str(maximum_full_tiles), str(partial_tile_pixels), str(total_tiles))
    return total_tiles
